...
class StudyCompassEngine:
    ...
    def _salary_label(self, salary_potential: Optional[float]) -> str:
        """Map numeric [0,1] to a neutral, relative label (no currency)."""
    ...
    def get_followup_context_for_ai(
        ...
        followup_type examples:
          ...
          - "expand_dream"
          - "market_info"  (opt-in only; includes salary_potential as RELATIVE label)
        ...
        # optional market info data (opt-in)
        salary_line = ""
        if followup_type == "market_info":
            f = self._field_by_name(target_name)
            salary_potential = None if not f else f.get("salary_potential", None)
            salary_line = (
                "\nמידע אופציונלי (שוק עבודה):\n"
                f"- פוטנציאל השתכרות: {self._salary_label(salary_potential)}\n"
                "- הערה: זה מדד יחסי פנימי בדאטה, לא הבטחה אישית ולא סכום בש״ח.\n"
            )
        ...
        if followup_type == "expand_recommendation":
            ...
            ctx.append("- תן הרחבה ניטרלית על שוק העבודה/יציבות/מסלולי כניסה. אל תעודד 'לרדוף אחרי כסף'. אל תתן מספרים בש״ח.")
        else:  # expand_dream
            ctx.append("- תן הרחבה על תחום החלום: וריאציות לימודיות, מסלולי כניסה, ומה אפשר לעשות כדי לבדוק התאמה בלי להתחייב.")

        if salary_line:
            ctx.append(salary_line)
